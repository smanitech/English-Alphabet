...
const LaunchRequestHandler = {
  ...
  handle(handlerInput) {
    // Get a random eaid for speechText
    const randomEngAlphabet = alphabetUtils.getRandomAlphabet(handlerInput);
    // Get prompt & reprompt speech
    const speakOutput = handlerInput.t('WELCOME_MESSAGE', { skillName: handlerInput.t('SKILL_NAME'), eaid: randomEngAlphabet.name });
...
/**
 * Handles EngAlphabetIntent or APL Touch Event (Alexa.Presentation.APL.UserEvent - eaidInstructions) requests sent by Alexa
 */
const EngAlphabetHandler = {
  canHandle(handlerInput) {
    return ((Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
      && Alexa.getIntentName(handlerInput.requestEnvelope) === 'EngAlphabetIntent')
      || (Alexa.getRequestType(handlerInput.requestEnvelope) === 'Alexa.Presentation.APL.UserEvent'
        && handlerInput.requestEnvelope.request.arguments.length > 0
        && handlerInput.requestEnvelope.request.arguments[0] === 'eaidInstructions'));
  },
  handle(handlerInput) {
    // Get slot Item
    const alphabetItem = alphabetUtils.getAlphabetItem(handlerInput.requestEnvelope.request);
    // Generate output to include a EngAlphabet with or without APL
    return EngAlphabetHandler.generateEngAlphabetOutput(handlerInput, alphabetItem);
  },
  generateEngAlphabetOutput(handlerInput, alphabetItem) {
    const { responseBuilder } = handlerInput;
    // eaid exists
    if (alphabetItem.id) {
      // Add l10n strings
      const alphabet = handlerInput.t('ALPHABET');
      const selectedEngAlphabet = alphabet[alphabetItem.id];
      Object.assign(alphabetItem, selectedEngAlphabet);
      // Add image
      alphabetItem.image = alphabetUtils.getAlphabetImage(alphabetItem.id);
      // Add a Card (displayed in the Alexa App)
      const cardTitle = handlerInput.t('DISPLAY_CARD_TITLE', { skillName: handlerInput.t('SKILL_NAME'), eaid: alphabetItem.name });
      ...
      // Spoken eaid does not exists
      // Add prompt : Is the Item slot is filled with a value ? 
      if (alphabetItem.spoken) {
        // Use spoken value to let the user knows no EngAlphabet exists for this value
        responseBuilder.speak(handlerInput.t('ALPHABET_NOT_FOUND_WITH_ITEM_NAME', { eaid: alphabetItem.spoken }));
      } else {
        // No spoken value
...
const HelpHandler = {
  ...
  handle(handlerInput) {
    ...
    const randomEngAlphabet = alphabetUtils.getRandomAlphabet(handlerInput);
    // Get prompt & reprompt speech
    const speakOutput = handlerInput.t('HELP_MESSAGE', { eaid: randomEngAlphabet.name });
    const repromptOutput = handlerInput.t('HELP_REPROMPT', { eaid: randomEngAlphabet.name });
    // add APL template if device is compatible
 ...
 * - IntentRequest - EngAlphabetIntent
 * - IntentRequest - HelpIntent
 * - LaunchRequest
 * - Alexa.Presentation.APL.UserEvent - eaidInstructions
 */
    ...
    while (actionnableHistory.length > 0) {
      ...
      if (replayRequest
        ...
        if ((replayRequest.type === 'IntentRequest'
          && replayRequest.intent.name === 'EngAlphabetIntent')
          ...
          // Get eaid item from request history not current Request
          const alphabetItem = alphabetUtils.getAlphabetItem(replayRequest);
          // Return JSON Output for EngAlphabet
          return EngAlphabetHandler.generateEngAlphabetOutput(handlerInput, alphabetItem);
        }
        if (replayRequest.type === 'IntentRequest'
  ...
  process(handlerInput, response) {
    ...
    switch (currentRequest.type) {
      case 'IntentRequest':
        ...
        recordRequest.actionable = (["EngAlphabetIntent", "AMAZON.HelpIntent"].includes(recordRequest.intent.name));
        break;
      case 'Alexa.Presentation.APL.UserEvent':
        recordRequest.arguments = currentRequest.arguments;
        recordRequest.actionable = (recordRequest.arguments[0] === 'eaidInstructions');
        break;
      case 'LaunchRequest':
...
exports.handler = Alexa.SkillBuilders.custom()
  ...
  .withCustomUserAgent('english-alphabet/v2')
  .lambda();
